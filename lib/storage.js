...
export function getResenasRef(negocioId) {
  if (!db) return null
  return collection(db, 'negocios', negocioId, 'resenas')
}

export async function getResenas(negocioId) {
  if (!db || !negocioId) return []
  try {
    const snap = await getDocs(getResenasRef(negocioId))
...
export async function agregarResena(negocioId, { usuarioId, nombre, rating, comentario }) {
  if (!db || !negocioId) return null
  try {
    const docRef = await addDoc(getResenasRef(negocioId), {
      usuarioId,
